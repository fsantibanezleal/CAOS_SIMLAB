...
| **joblib** | live | ✓ | Pure-Python; the Monte-Carlo replications. (In Pyodide it runs the **threading** backend in-Worker — loky uses subprocesses, which WASM cannot spawn, so S10 hard-codes `backend="threading"`; the speedup is modest but the *result* is identical to the offline run, which is all replay = truth needs.) |
...
| **OR-Tools** | precompute | ✗ | C++ with no WASM build; `pure_python=False`. CP-SAT (S06), Routing (S08), GLOP (S11) re-solve in `run()`, so they are precompute-only. **S07 is the exception:** its CP-SAT/NetworkX route *plan* is precomputed offline and **committed as data** (`s07_plans.py`), so its `run()` carries only the pure-Python SimPy replay — S07 runs **live** (`pure_python=True`, `wheels=["numpy","simpy"]`), with OR-Tools never reached in the worker. |
