...
| [**S04 — Emergency Department Patient Flow**](../../use-cases/04_s04_ed.md) | **SimPy** (multi-stage, priority queue) | no closed form → face validity + sensitivity | live | Non-stationary (hour-of-day) Poisson arrivals, priority triage, resource-limited multi-stage flow (triage → treatment → disposition). One seeded run (the replications/CI honesty lesson is **S10**). |
