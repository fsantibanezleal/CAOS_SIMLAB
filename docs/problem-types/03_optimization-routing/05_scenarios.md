...
| **[S07 — Construction Haul Routing](../../use-cases/07_s07_haul.md)** | **NetworkX + OR-Tools CP-SAT** route plan, precomputed offline and committed as data (grade×wall grid) | **SimPy** replay (live) under stochastic load/dump/delay; 2D grade overlay (3D terrain post-v1) | Optimize-then-simulate where **elevation drives the cost model**; the plan is committed, the replay runs live |
