...
optimization half; each commits its **solver search parameters and a deterministic stopping rule** (plus a
fixed seed where the engine is seeded) into the manifest so the replayed plan is reproducible (the
...
limit) — here OR-Tools Routing is single-threaded GLS with **no** seed, and only the PyVRP contrast carries a
fixed seed. S09 is the exception — it has no optimizer (see the S09 row and footnote).
...
| **[S08 — Vehicle Routing (CVRP)](../../use-cases/08_s08_vrp.md)** | OR-Tools Routing (`GUIDED_LOCAL_SEARCH` + `solution_limit = 200`, single-thread, **no seed**; minimizes distance + global-span balance) **+ PyVRP** HGS (SOTA contrast, pure total distance, fixed `seed = 42`) | — (no SimPy; static plans replayed as vehicles driving the network) | The OR-Tools-vs-PyVRP total-distance gap on the identical CVRP instance (strictly CVRP — no time windows, no DES leg) |
