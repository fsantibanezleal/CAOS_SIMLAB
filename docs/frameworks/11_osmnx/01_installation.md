...
layer** for the geospatial routing scenarios — see [`02_usage.md`](./02_usage.md) and
[`03_applying.md`](./03_applying.md). The wiki landing page for this node is
[`../11_osmnx.md`](../11_osmnx.md).
...
> do not need to run `pip` to use it here. Verify with the snippet at the bottom of this page.

## Which requirements file it belongs to

OSMnx is a **precompute-lane** dependency. It belongs in `requirements-precompute.txt` —
...
**Honesty note.** In the current repo this line is **commented out**, because **no scenario
uses real OpenStreetMap data yet** — the routing scenarios run on a self-contained synthetic
`GridNetwork` (`simlab/scenarios/_geo.py`). OSMnx is installed and documented here as the
*planned* drop-in road layer for S07/S09 (and as the contrast to the synthetic graph). When a
scenario first ingests OSM, uncomment this line and pin it. The installed version (2.1.0) is
newer than the `>=1.9` floor and is the one these docs target.

It is **not** in:

- `requirements.txt` (core, live/browser tier) — OSMnx's native geospatial dependencies
  (GEOS/PROJ/GDAL via geopandas) cannot run in the Pyodide/WASM browser tier; and
- `requirements-gpu.txt` (CUDA) — OSMnx is pure-Python and CPU-only (see *CUDA / GPU notes*).
...
- **Network access at runtime, not install time.** Installing OSMnx needs no network beyond
  ...
  the offline strategy in [`02_usage.md`](./02_usage.md).
...
Expected output is captured verbatim in [`02_usage.md`](./02_usage.md#4-verified-output).

---

**Next:** [`02_usage.md`](./02_usage.md) — concepts, the 2.x API, and the worked example.
Then [`03_applying.md`](./03_applying.md) — how it fits the routing scenarios and when to pick it.
