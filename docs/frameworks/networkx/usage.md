...
under scenarios **S07** (construction haul routing) and **S09** (ambulance dispatch).
...
| Directed graph | `nx.DiGraph()` | one-way edges (e.g. one-way streets) |
...
This matches the lab's "replay = truth" contract: a committed routing trace must reproduce
...
```bash
.venv/Scripts/python.exe docs/frameworks/networkx/example.py
```

Captured stdout (real run, deterministic — identical across repeated runs):
...
- **All-pairs / a cost matrix to feed a router:** `dict(nx.all_pairs_dijkstra_path_length(g,
  weight="weight"))` builds the N×N travel-time matrix that OR-Tools or PyVRP consume. Per the
  research, *the matrix — not the solver — is usually the real bottleneck*; cap live instances small
  (≈ ≤ 20–30 stops) and precompute the matrix for anything larger.
