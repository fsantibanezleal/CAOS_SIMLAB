...
This page covers the key concepts and API, then walks a minimal runnable example and shows its **real
captured output**.
...
The script [`example.py`](./example.py) is self-contained and seeded. It:

1. Draws a sample of per-run KPIs from a small M/M/c queue (the same `mmc_mean_wait` shape as
   `simlab/scenarios/s10_montecarlo.py`), at three sizes `K ∈ {8, 30, 400}`.
2. Computes both the normal (z) and Student-t 95% CIs with `scipy.stats`, showing the one-call
   `.interval(...)` form agrees with the explicit `.ppf(...) × sem` form.
3. Prints the critical values, a comparison table (note the `t-width / z-width` ratio shrinking toward
   `1.000` as `K` grows), and spells out the small-`K=8` case where the t-interval is meaningfully wider.
4. Confirms determinism: the same base seed reproduces the sample exactly.
...
(Python 3.13.0, scipy 1.18.0, numpy 2.4.6). This is the real stdout, not a reconstruction:
