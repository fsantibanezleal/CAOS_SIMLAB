...
[GPU-lane guide](../../guides/03_gpu-lane.md) for the lane contract.

This page covers the exact pin, the requirements lane, the transitive dependencies, and the platform / CUDA
caveats. For the API and the walked-through example go to [`02_usage.md`](./02_usage.md); for when to reach
for CuPy versus the CPU default go to [`03_applying.md`](./03_applying.md).
...
`cupy-cuda12x` (not the source-build `cupy` package, and not the CUDA-11 `cupy-cuda11x`):
...
> documented so the exact pin is on record. If you are on a CUDA-11 machine you would instead need
> `cupy-cuda11x`; if you build from source (rarely needed) the package is plain `cupy` and requires a local
> `nvcc`.
...
| Python | 3.13 (the repo's `.venv`) |

## Which requirements file it belongs to

[`requirements-gpu.txt`](../../../requirements-gpu.txt) — the **optional, post-v1, local-only GPU lane**. The
header of that file states the contract plainly:

- it is **never a hard dependency**, and **never on the deploy path** (the app is static; the contract is the
  committed trace, not the GPU);
...
CuPy is therefore deliberately **absent** from:

- the base [`requirements.txt`](../../../requirements.txt) — the browser/Pyodide wheel closure, kept to
  `numpy` + `simpy` so a GPU-less learner can clone and run the whole live lane; and
- [`requirements-precompute.txt`](../../../requirements-precompute.txt) — the CPU precompute pipeline
  (OR-Tools, joblib, etc.).

Keeping a native CUDA dependency out of the base and precompute lanes is exactly what lets a GPU-less learner
clone and run everything; the GPU is strictly additive.
...
etc.) are **not** Python packages — they come from the system CUDA toolkit / driver. That is precisely why a
...
- **The `CUDA path could not be detected` warning is expected here.** On *first import*, if the CUDA toolkit
  ...
  This is **informational, not fatal** — it means CuPy could not locate a CUDA install. The example treats it
  as the expected symptom of "no usable GPU on this machine" and falls back to NumPy. If you *do* have a
...
- **Toolkit headers, not just a device, are required.** A subtle real-world failure (observed while verifying
  this guide): a machine can report a CUDA device — `cupy.cuda.runtime.getDeviceCount()`, cuRAND draws, and
  `getDeviceProperties()` all succeed — yet still lack the CUDA toolkit *headers* that CuPy needs to
  **JIT-compile elementwise kernels** (via NVRTC). The failure then surfaces only when the first array
  arithmetic op compiles:
...
- **Compute Capability:** CUDA 12.x supports NVIDIA GPUs of compute capability ≥ 5.0 (Maxwell and newer). The
  lab's reference local GPU is an RTX 4070 Laptop (8 GB) — comfortable for batched Monte-Carlo (fp32/fp64
...
- **Driver vs toolkit:** the NVIDIA *driver* must be new enough for CUDA 12.x; the *toolkit* (headers + NVRTC)
  must be present for kernel JIT. The wheel ships the CUDA *runtime* libraries but relies on the system for
  the driver and (for JIT) the headers.

No secrets, no API keys, and no network access are involved in installing or running CuPy.

---

**Next:** [`02_usage.md`](./02_usage.md) — the API, the example walked through, and its real captured output.
