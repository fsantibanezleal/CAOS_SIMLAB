...
evacuated. It walks through the minimal example in [`example.py`](example.py) and shows its
**real captured output** (re-run for this revision).

> Prerequisite: [01_installation.md](01_installation.md). Next: [03_applying.md](03_applying.md).
...
sim = jps.Simulation(model=jps.CollisionFreeSpeedModel(), geometry=ROOM, dt=0.01)
...
- `iterate()` advances exactly one `dt`. `iteration_count()` returns the number of steps so
...
- **Agents too close at spawn.** Placement closer than `2 * radius` raises
  `RuntimeError: Model constraint violation: Agent ... too close to agent ...`. Lay agents
  on a grid with spacing comfortably above `2 * radius` (0.4 m for the default 0.2 m
...
- **Forgetting the journey / stage ids.** `journey_id` and `stage_id` on the agent params
  ...
  destination.
- **Expecting an RNG seed on the model.** There isn't one; control reproducibility via your
  own placement RNG (the model is already deterministic).
