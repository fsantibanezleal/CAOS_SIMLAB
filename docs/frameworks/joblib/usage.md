...
2. **Per-task seeding inside the worker.** Each replication must own an *independent* RNG stream. The lab's
   rule (see `simlab/core/rng.py`) is one `np.random.default_rng(seed)` per run. With joblib you pass the
   *seed* into the task and build the generator inside the worker, so workers never share a global RNG. This
   is what guarantees the replications are genuinely i.i.d. and that the study is reproducible.
...
1. **The cheap stochastic function — one replication.**
   ...
   ship it to worker processes. This mirrors `simlab/scenarios/s10_montecarlo.py::mmc_mean_wait`.
...
```
.venv/Scripts/python.exe docs/frameworks/joblib/example.py
```
...
- **The CI brackets the theory.** At ρ≈0.67 the 400-replication mean (`0.4346`) lands close to the
  ...
  pulls the CI *below* theory — see `applying.md` and the S10 docstring; that is a model/methodology lesson,
  not a joblib issue.
...
- **Forgetting the `__main__` guard on Windows** causes an endless process-spawn loop with the `loky`
  backend. → Always guard the entry point (`example.py` does).
