# NetLogo Web (Tortoise) — usage

How to take a NetLogo model that has been exported to **standalone HTML** (see
[installation.md](./installation.md)) and embed it as a **LIVE, client-side simulation card** inside the
lab's React/Vite SPA — with the IDE chrome stripped so the visitor sees only the running model.

> **No Python example here.** NetLogo Web is JavaScript, not a pip pipeline, so there is **no `example.py`**
> to run and **no "Verified output" stdout block** like the Python frameworks have. Instead this page gives
> the real **HTML/JS embed snippet** you drop into the SPA, plus what to expect on screen. The facts about
> the export format and license model are grounded in NetLogo's own docs (see References).
...
- **Sliders** that change behavior live (e.g. Schelling's `%-similar-wanted`, SIR's infection probability).
- **Plots** updating each tick (e.g. SIR's S/I/R curves).
...
HTML export — re-export per [installation.md](./installation.md) Step 1.
...
- **License split is real**: *Code Examples* in the Models Library are **CC0 / public domain**; most other
  ...
  the model-sourcing rule in [applying.md](./applying.md) §4.
