...
← Previous: [01 · Installation](./01_installation.md) · node: [Numba](../14_numba.md) ·
next: [03 · Applying it](./03_applying.md).
...
A practical note on the "no-python" contract: when Numba *cannot* compile something in
nopython mode it raises a `TypingError` at compile time rather than silently falling back to
the interpreter (the old `@jit` object-mode fallback is deprecated). Treat that error as a
signal to push the unsupported Python (object creation, I/O, dynamic typing) *out* of the
kernel and feed the kernel plain arrays and scalars — which is also what keeps the kernel a
...
nothing; it writes results into an output array you copy back with `.copy_to_host()`. The
launch is *asynchronous* — the host call returns before the GPU finishes — so any timing or
result read must follow the `.copy_to_host()` (which synchronises), never precede it.
...
is a bug, not a result. This *validation against a closed form* is the lab's standard way of
proving a stochastic estimator is correct before trusting it where no closed form exists.
