# OSMnx — Applying it to a real problem

This page explains *where* OSMnx fits in CAOS_SIMLAB, the **pattern** it participates in, the
**honest trade-offs** (grounded in research dimension 03, *Optimization & Routing*), and **when to
pick OSMnx vs the alternatives**. For the API and verified example see [`usage.md`](./usage.md);
for install/version see [`installation.md`](./installation.md).
...
## 3. The pattern: build-the-graph → matrix → optimize → simulate
...
## 5. When to pick OSMnx vs the alternatives
