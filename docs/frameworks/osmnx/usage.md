...
For install details and the requirements-file placement, see [`installation.md`](./installation.md).
For how this plugs into our scenarios and when to pick it, see [`applying.md`](./applying.md).
...
3. **Snap.** `ox.distance.nearest_nodes(G, X=..., Y=...)` snaps the map centre coordinate to the
   nearest junction — node 4, the centre of the grid, as expected.
...
4. **Route.** `ox.routing.shortest_path` is run from node 0 (SW corner) to node 8 (NE corner),
   ...
   speed, both weightings pick the same path here — a deliberately clean teaching case.
