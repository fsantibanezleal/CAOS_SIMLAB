...
- **run time** < 3000 ms — the committed gate timings are inside the cap (the cold first run is the largest,
  in the low-seconds range). The exact per-variant `gate.run_ms` is measured and recorded in the manifest — it
  varies with host load, so read it there rather than a hard-coded figure.
