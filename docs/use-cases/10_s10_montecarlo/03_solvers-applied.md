...
|---|---|---|
| **Precompute** (committed gallery) | the offline `.venv` runs `joblib + scipy` to generate the seed-42 CI sweep; the result is committed as a deterministic chart trace | full native joblib (`threading`) + full `scipy.stats`; this is where the dedicated engines live |
| **Live** (Pyodide, in-browser) | the same pure-Python, fully-seeded model runs on demand | `joblib` + `scipy` are heavy native deps, so they are **imported lazily inside `run()`** (never at module import) — the registry import and the whole live lane work even when they are absent; the `threading` backend is the only one that works under WASM |
