...
export const ARCH_TABS: ArchTab[] = [
  {
    ...
    body: {
      en: [
        ...
        "Design-build lifecycle: research and decide the right tool per problem → implement the scenario once in the shared engine simlab/scenarios/<id>.py → run the precompute pipeline in the local .venv → commit the seeded trace + manifest (git-as-data) → build the SPA (Vite) → publish to GitHub Pages. One engine drives the notebooks, the offline pipeline and the live web app, so what is learned is what runs.",
        "The shared simlab/ package + its committed artifacts are the product; the web app replays them and, for the light scenarios, re-runs the very same Python in the browser.",
      ],
      es: [
        "CAOS_SIMLAB es un laboratorio público, didáctico y guiado por bibliotecas para cuatro familias de simulación — eventos discretos (DES), basada en agentes (ABM), optimización / investigación de operaciones y Monte-Carlo. Cada uno de los 11 casos se resuelve con la herramienta dedicada correcta para su tipo de problema (SimPy, Ciw, Mesa 3, OR-Tools, PyVRP, NetworkX, joblib, SciPy), nunca una imitación casera.",
        "Ciclo de diseño-construcción: investigar y decidir la herramienta correcta por problema → implementar el escenario una vez en el motor compartido simlab/scenarios/<id>.py → ejecutar el pipeline de precómputo en el .venv local → registrar en git la traza sembrada + el manifest (git-as-data) → construir la SPA (Vite) → publicar en GitHub Pages. Un solo motor mueve los notebooks, el pipeline offline y la app web, así que lo que se aprende es lo que se ejecuta.",
        "El paquete compartido simlab/ + sus artefactos registrados son el producto; la app web los reproduce y, para los escenarios livianos, re-ejecuta el mismo Python en el navegador.",
      ],
    },
  },
  {
    ...
    body: {
      en: [
        "The core design is two lanes, and the choice is measured, not guessed. The live lane (web): a Pyodide Web Worker loads numpy/pandas/scipy/networkx/sqlite3 and micropip-installs simpy/ciw/mesa/joblib, then runs the real simlab engine via simlab.live.run_trace_json — so the eight pure-Python scenarios (s01–s05, s07, s09, s10) recompute in the browser as the slider moves.",
        "The offline / compute lane (.venv): the precompute pipeline runs the native solvers that have no WebAssembly build — OR-Tools CP-SAT (S06), OR-Tools Routing + PyVRP (S08), GLOP LP (S11), and the S07 NetworkX + OR-Tools route plan — and writes a compact seeded trace + manifest. The GPU lane (CuPy / Numba / Taichi / JAX) is documented but intentionally out of scope.",
        "The replay path is the shared fallback: every scenario ships a committed seeded trace, so the page paints instantly while Pyodide warms, and the native scenarios replay that trace exactly. The 4-gate rule (classify_lane) decides live vs precompute from a real measurement and records it in each scenario's manifest.",
      ],
      es: [
        "El diseño central son dos carriles, y la elección se mide, no se adivina. LIVE (web): un Web Worker de Pyodide carga numpy/pandas/scipy/networkx/sqlite3 e instala con micropip simpy/ciw/mesa/joblib, y luego ejecuta el motor real simlab vía simlab.live.run_trace_json — así los ocho escenarios Python-puros (s01–s05, s07, s09, s10) recomputan en el navegador al mover un control.",
        "El carril offline / cómputo (.venv): el pipeline de precómputo ejecuta los solvers nativos sin build de WebAssembly — OR-Tools CP-SAT (S06), OR-Tools Routing + PyVRP (S08), GLOP LP (S11) y el plan de ruta NetworkX + OR-Tools de S07 — y escribe una traza sembrada compacta + manifest. El carril GPU (CuPy / Numba / Taichi / JAX) está documentado pero intencionalmente fuera de alcance.",
        "El carril de replay es el respaldo común: cada escenario embarca una traza sembrada registrada, así la página pinta al instante mientras Pyodide calienta, y los escenarios nativos reproducen esa traza exactamente. La regla de 4 compuertas (classify_lane) decide live vs precómputo a partir de una medición real y lo registra en el manifest de cada escenario.",
      ],
    },
  },
  {
    ...
    body: {
      ...
      es: [
        ...
        "En el build, el hook prebuild copy-data.mjs superpone los data/artifacts + manifests registrados e inlinea cada simlab/**/*.py en pyodide/simlab-sources.json (para que el carril live ejecute el código de motor exacto). Luego deploy-pages.yml construye y publica el sitio estático — sin backend, sin VPS.",
      ],
    },
  },
  {
    ...
    body: {
      ...
      es: [
        ...
        "Monte-Carlo — S10 ejecuta N réplicas independientes con joblib y forma un intervalo de confianza al 95% con SciPy (el z exacto = norm.ppf(0.975)), mostrando la banda estrecharse como 1/√n. En todas las familias el contrato es replay determinista: una corrida es función pura de (params, seed), así la traza registrada es la fuente de verdad y la re-ejecución en el navegador la iguala exactamente.",
      ],
    },
  },
  {
