...
const en = {
  ...
  live: {
    ...
    nativeBody: "This scenario's engine or wheel closure can't run in the browser worker — a native solver (OR-Tools) and/or a heavy Python closure (Mesa, joblib, SciPy, NetworkX) — so it runs in the local pipeline and the seeded trace is replayed here. Use the Simulator tab to compare the pre-simulated regimes (details below).",
...
const es: typeof en = {
  ...
  live: {
    ...
    nativeBody: "El motor o el cierre de wheels de este escenario no puede correr en el worker del navegador — un solver nativo (OR-Tools) y/o un cierre Python pesado (Mesa, joblib, SciPy, NetworkX) — así que corre en el pipeline local y aquí se reproduce el trace sembrado. Usa la pestaña Simulador para comparar los regímenes pre-simulados (detalles abajo).",
