...
const es: typeof en = {
  ...
  arch: {
    ...
    subtitle: "Qué se ejecuta dónde, el motor y los contratos de datos — todo el sistema de un vistazo.",
    open: "Arquitectura / cómo funciona",
    close: "Cerrar",
    footer: "Un motor offline y en vivo; la traza registrada es la fuente de verdad — replay determinista.",
  },
  ...
  sim: {
    ...
    tabContext: "Contexto",
    tabLive: "En vivo (el navegador)",
  },
  live: {
    intro: "Ajustar los parámetros y ejecutar el escenario real de Python aquí mismo: se ejecuta en el navegador con Pyodide (WebAssembly), sin servidor. El resultado se anima con el mismo reproductor.",
    ...
    verify: "Verificar contra la traza registrada",
    ...
    nativeBody: "El motor de este escenario es un solver de código nativo (OR-Tools) sin build de WebAssembly, así que no puede ejecutarse en el worker del navegador: se ejecuta en el pipeline local y aquí se reproduce la traza sembrada. La pestaña Simulador permite comparar los regímenes pre-simulados (detalles abajo).",
    verifyMatch: "✓ Reproduce exactamente la traza registrada — replay = verdad.",
    verifyMismatch: "Difiere levemente de la traza registrada (versión del runtime/biblioteca del navegador) — sigue siendo una corrida válida y reproducible.",
  ...
  compare: {
    ...
    note: "Espera media simulada (puntos) contra la teoría cerrada M/M/c (línea), en cada régimen pre-simulado. Al hacer clic en un punto se carga ese régimen arriba.",
