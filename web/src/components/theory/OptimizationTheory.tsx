      ...
      content: (
        <div className="th-block">
          <div className="prose">
            ...
            <p>
              {es
                ? "Dos familias de algoritmos lo resuelven. El método símplex (Dantzig) camina de vértice en vértice por las aristas del poliedro, mejorando el objetivo en cada paso hasta que ningún vecino mejora — entonces ese vértice es óptimo. Es exponencial en el peor caso (Klee–Minty) pero extraordinariamente rápido en la práctica. Los métodos de punto interior (Karmarkar 1984) atraviesan el interior del poliedro siguiendo una trayectoria central y convergen al óptimo en tiempo polinómico demostrable; ganan en problemas muy grandes y dispersos. OR-Tools expone el símplex a través de GLOP, su solver LP de punto flotante — exactamente el motor que SimLab usa para el plan de mezcla de S11 — y los métodos de punto interior a través de su solver aparte, PDLP."
                : "Two algorithm families solve it. The simplex method (Dantzig) walks from vertex to vertex along the edges of the polyhedron, improving the objective at each step until no neighbour improves — that vertex is then optimal. It is worst-case exponential (Klee–Minty) yet extraordinarily fast in practice. Interior-point methods (Karmarkar 1984) cut through the interior of the polyhedron along a central path and converge to the optimum in provable polynomial time; they win on very large, sparse problems. OR-Tools exposes simplex through GLOP, its floating-point LP solver — exactly the engine SimLab drives for the S11 blend plan — and interior-point/first-order methods through its separate PDLP solver."}
            </p>
            <p>
              {es
...
\sum_i x_i = D,\ \ \sum_i g_i x_i - g^{*}D = d^{+}-d^{-},\ \ 0\le x_i\le a_i,\ \ d^{+},d^{-}\ge 0.`}
          ...
          <Callout variant="strong" title={es ? "Rol en el modelado" : "Modeling role"}>
            <p>
              {es
                ? "GLOP resuelve esta LP en el carril de precómputo de S11. El vector solución plan_x = (x_i) es el plan de flujo que la flota debe realizar; plan_grade = (Σ g_i x_i)/D_ef es la ley óptima alcanzable, donde el código usa la demanda efectiva D_ef = min(D, Σ a_i) (acotada por la disponibilidad total; en cada variante embarcada Σ a_i ≥ D, así que D_ef = D). La simulación de ejecución luego mide cuánto de ese óptimo entrega realmente una flota fija."
                : "GLOP solves this LP in the S11 precompute lane. The solution vector plan_x = (x_i) is the flow plan the fleet must realize; plan_grade = (Σ g_i x_i)/D_eff is the optimal achievable grade, where the code uses the effective demand D_eff = min(D, Σ a_i) (capped by total availability; for every shipped variant Σ a_i ≥ D, so D_eff = D). The execution simulation then measures how much of that optimum a fixed fleet actually delivers."}
            </p>
          </Callout>
...
\bigl(x_j\le\lfloor\bar{x}_j\rfloor\bigr)\ \ \text{or}\ \ \bigl(x_j\ge\lceil\bar{x}_j\rceil\bigr).`}
          ...
          <Callout variant="note" title={es ? "Rol en el modelado" : "Modeling role"}>
            <p>
              {es
                ? "Es el marco que sustenta S08 (VRP) y la formulación alternativa MILP de S06 (job-shop). SimLab usa la maquinaria entera de OR-Tools (CP-SAT y el solver de rutas) precisamente porque estas decisiones — qué arco recorre cada vehículo, en qué orden se ejecuta cada operación — son indivisibles."
        ...
        <div className="th-block">
          ...
          <div className="assume">
            ...
            <ul>
              {[
                es
                  ? "Determinismo: duraciones fijas, máquinas siempre disponibles, sin averías ni preparaciones. S06 siembra las instancias y ejecuta CP-SAT con un trabajador y semilla fija — el trazo es replicable byte a byte."
                ...
                es
                  ? "No apropiación: una vez iniciada, una operación se ejecuta hasta terminar (la no-overlap de S06 lo asume); el job-shop apropiativo es un modelo distinto."
                ...
                es
                  ? "Optimalidad demostrable pero no garantizada en cualquier plazo: CP-SAT se ejecuta con límite de tiempo (10 s en S06). En instancias mayores puede devolver FEASIBLE (factible, no probado óptimo) — el KPI optimal lo distingue."
      ...
      content: (
        <div className="th-block">
          <div className="prose">
            ...
            <p>
              {es
                ? "S08 modela exactamente esto y ejecuta la misma instancia capacitada por dos solvers SOTA en paralelo. El plan primario lo da el solver de rutas de OR-Tools: los clientes están en una grilla; la matriz de distancias D[a][b] son caminos mínimos de Dijkstra entre nodos especiales (depósito + clientes), escalados a enteros. Una dimensión de capacidad (AddDimensionWithVehicleCapacity) impone el límite por vehículo; una dimensión de distancia con un coeficiente de costo de span global (SetGlobalSpanCostCoefficient) penaliza la ruta más larga, equilibrando las cargas. El VRP es demasiado grande para resolverse de forma exacta a escala, así que OR-Tools usa metaheurísticas: una solución inicial codiciosa (PATH_CHEAPEST_ARC) refinada por búsqueda local guiada (GUIDED_LOCAL_SEARCH), que penaliza progresivamente los rasgos que aparecen en óptimos locales para escapar de ellos. El criterio de parada no es de reloj: es un límite determinista de cantidad de soluciones (solution_limit = 200) con semilla fija — no hay time_limit. Una parada por conteo es independiente de la máquina (un límite de reloj dejaría que una CPU rápida explore más), así que la traza registrada es byte-estable en cualquier equipo. Como contraste, la misma instancia se resuelve también con PyVRP (Hybrid Genetic Search, el estado del arte abierto en VRP, Wouda, Lan & Kool 2024), que minimiza la distancia total pura: su plan suele ser más corto en total pero menos balanceado, y la brecha de distancia OR-Tools vs. PyVRP es justamente lo que el escenario expone (en el campo analytic de la traza)."
                : "S08 models exactly this and runs the same capacitated instance through two SOTA solvers side by side. The primary plan comes from the OR-Tools routing solver: customers sit on a grid; the distance matrix D[a][b] holds Dijkstra shortest paths between special nodes (depot + customers), scaled to integers. A capacity dimension (AddDimensionWithVehicleCapacity) imposes the per-vehicle limit; a distance dimension with a global-span cost coefficient (SetGlobalSpanCostCoefficient) penalizes the longest route, balancing loads. The VRP is too large to solve exactly at scale, so OR-Tools uses metaheuristics: a greedy initial solution (PATH_CHEAPEST_ARC) refined by guided local search (GUIDED_LOCAL_SEARCH), which progressively penalizes features appearing in local optima to escape them. The stopping rule is not wall-clock: it is a deterministic solution-count limit (solution_limit = 200) with a fixed seed — there is no time_limit. A solution-count limit is machine-independent (a wall-clock limit would let a faster CPU explore more), so the committed trace is byte-stable on any machine. As a contrast, the same instance is also solved with PyVRP (Hybrid Genetic Search, the open VRP state of the art, Wouda, Lan & Kool 2024), which minimizes pure total distance: its plan is typically shorter in total but less balanced, and the OR-Tools vs. PyVRP distance gap is exactly what the scenario surfaces (in the trace's analytic field)."}
            </p>
          </div>
          ...
          <div className="assume">
            ...
            <ul>
              {[
                es
                  ? "Las metaheurísticas no garantizan optimalidad: S08 reporta la mejor ruta hallada al alcanzar la parada determinista (OR-Tools: solution_limit = 200 soluciones, no un límite de reloj; PyVRP: MaxIterations = 200). En instancias pequeñas suele ser óptima; en grandes, casi óptima sin prueba."
                  : "Metaheuristics do not guarantee optimality: S08 reports the best route found when the deterministic stop is reached (OR-Tools: solution_limit = 200 solutions, not a wall-clock limit; PyVRP: MaxIterations = 200). On small instances it is usually optimal; on large ones, near-optimal without proof.",
                es
                ...
                es
                  ? "S08 es optimización pura (no estocástica) y estrictamente CVRP — sin ventanas de tiempo (no es VRPTW) y sin pata de simulación de eventos discretos: instancias sembradas, ambos solvers con parada determinista por conteo (no de reloj), trazo precalculado que la web reproduce como vehículos recorriendo la red."
                  : "S08 is pure optimization (not stochastic) and strictly CVRP — no time windows (not VRPTW) and no discrete-event simulation leg: seeded instances, both solvers with a deterministic count-based stop (not wall-clock), a precomputed trace the web replays as vehicles driving the network.",
              ].map((x, i) => (
                <li key={i}>{x}</li>
              ))}
            </ul>
...
\sum_{j} x_{ij}=1\ \forall i,\ \ \sum_{i} x_{ij}=1\ \forall j\quad(\text{degree}),\qquad x_{ij}\in\{0,1\}.`}
          ...
          <Callout variant="strong" title={es ? "Rol en el modelado" : "Modeling role"}>
            <p>
              {es
                ? "S08 es el escenario VRP del laboratorio. Ejecuta la misma instancia por OR-Tools (primario: equilibra cargas vía span global) y PyVRP (contraste SOTA: distancia total pura), y sus KPIs (total_distance, vehicles_used, max_route_time) hacen visible la brecha de distancia entre ambos. Cuidado con la intuición: dar más vehículos no siempre ayuda — en estas instancias las variantes manyveh (4 veh) y c15v4 (4 veh) dejan el vehículo extra sin usar (vehicles_used=3, misma distancia total y misma ruta más larga que base y c15 con 3 vehículos): cuando 3 rutas ya cubren la demanda al mínimo, el solver no parte una cuarta. El trazo óptimo se reproduce como vehículos coloreados recorriendo la red."
                : "S08 is the lab's VRP scenario. It runs the same instance through OR-Tools (primary: balances loads via the global span) and PyVRP (SOTA contrast: pure total distance), and its KPIs (total_distance, vehicles_used, max_route_time) make the distance gap between the two visible. Mind the intuition: handing the solver more vehicles does not always help — on these instances the manyveh (4 veh) and c15v4 (4 veh) variants leave the extra vehicle unused (vehicles_used=3, identical total distance and identical longest route to base and c15 with 3 vehicles): once 3 routes cover the demand at minimum cost, the solver does not open a fourth. The optimal trace replays as colored vehicles driving the network."}
            </p>
          </Callout>

          <Refs ids={["dantzig1959vrp", "tothvigo2014", "applegate2006", "voudouris1999", "pyvrp2024", "ortools"]} label={es ? "Referencias:" : "References:"} />
        </div>
      ),
    },
    {
...
\text{Dijkstra, LP, TU-assignment}\in\text{P},\quad \text{job-shop, TSP, VRP}\in\text{NP-hard}.`}
          ...
          <Callout variant="note" title={es ? "Rol en el modelado" : "Modeling role"}>
            <p>
              {es
                ? "Esta sub-pestaña justifica por qué S06/S08/S11 viven en el carril de precómputo (solver nativo, NP-difícil, determinista) mientras los simuladores estocásticos de Python puro se ejecutan en vivo. La frontera de complejidad es la frontera de la arquitectura."
