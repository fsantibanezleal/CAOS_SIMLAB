      ...
      content: (
        <div className="prose">
          ...
          <div className="assume">
            ...
            <ul>
              {[
                ...
                es
                  ? "Estimar-y-luego-probar sobre los mismos datos infla el rechazo; con n grande todo modelo paramétrico es rechazado. Usar gráficos Q–Q y criterio; reservar un conjunto de validación cuando sea posible."
          ...
          <figure className="figure">
            ...
            <figcaption className="fig-cap">
              {es
                ? "Ajustar y luego verificar: se superpone la exponencial ajustada y se lee la brecha K–S Dₙ = supₓ|Fₙ(x)−F(x)| sobre la CDF."
    ...
    // ───────────────────────────────────────────────────────────────────────
    {
      ...
      content: (
        <div className="prose">
          ...
          <p>
            {es
              ? "La reproducibilidad es innegociable: un resultado de simulación que no se puede regenerar no es un resultado. En la práctica se siembra cada corrida explícitamente y se registra la semilla (las guías STRESS lo exigen como ítem de reporte). Pero hay una sutileza más allá de una única semilla global. Al ejecutar muchas réplicas, o al comparar dos configuraciones, conviene que los flujos que alimentan partes distintas del modelo sean independientes y sin solapamiento —compartir un único generador global entre «llegadas» y «servicio» arriesga correlación accidental, y resembrar de forma ingenua puede situar dos réplicas en partes solapadas del período. La solución limpia es un generador con flujos nombrados (uno por proceso estocástico) y subflujos (uno por réplica): exactamente para lo que se diseñó MRG32k3a, y lo que hoy ofrece numpy.random.Generator con SeedSequence.spawn() en Python."
    ...
    // ───────────────────────────────────────────────────────────────────────
    {
      ...
      content: (
        <div className="prose">
          ...
          <p>
            {es
              ? "El error de principiante más común en simulación estocástica es ejecutar el modelo una vez, leer el tiempo de espera promedio y reportarlo como «la» respuesta. No lo es: una sola corrida es una muestra de un proceso aleatorio, y su promedio es a su vez una variable aleatoria con error de muestreo. Ejecutar el mismo modelo con otra semilla da otro número. El remedio es el cimiento del análisis de salidas: las réplicas independientes. Se ejecuta el modelo n veces con flujos de números aleatorios independientes (subflujos distintos, no solo semillas distintas), se recoge el estadístico resumen por corrida X₁,…,Xₙ (p. ej. la espera media en cola de cada corrida) y se tratan como una muestra i.i.d. Como los Xᵢ son independientes e idénticamente distribuidos entre réplicas, la estadística clásica se aplica directamente a ellos —aunque las observaciones dentro de una corrida estén fuertemente autocorrelacionadas (las esperas de clientes consecutivos lo están), los resúmenes entre corridas no lo están."
              : "The single most common beginner error in stochastic simulation is to run the model once, read off the average waiting time, and report it as \"the\" answer. It is not: a single run is one sample from a random process, and its average is itself a random variable with sampling error. Running the same model with a different seed gives a different number. The remedy is the bedrock of output analysis: independent replications. Run the model n times with independent random-number streams (different substreams, not just different seeds), collect the per-run summary statistic X₁,…,Xₙ (e.g. each run's mean queue wait), and treat those as an i.i.d. sample. Because the Xᵢ are independent and identically distributed across replications, the classical statistics apply directly to them — even though the within-run observations are heavily autocorrelated (consecutive customers' waits are correlated), the between-run summaries are not."}
          </p>
          ...
          <p>
            {es
              ? "Quién ejecuta este marco de réplicas en el laboratorio importa, y es honesto ser preciso: la corrida M/M/c en vivo de S01 (SimPy) es una sola corrida de ~300 clientes —produce un W_q puntual, sin réplicas ni IC. El estudio replicado-con-IC propiamente tal es S10 (Monte-Carlo sobre joblib + SciPy): ejecuta n réplicas independientes por régimen y dibuja la banda del IC alrededor de la media junto a la línea Erlang-C, y el aprendiz observa cómo la banda se estrecha al crecer n (deslizador), la ley 1/√n en acción, y dónde cae el valor analítico. S01 lleva además una verificación cruzada de segundo motor con Ciw (un estudio acotado de 10 réplicas que reporta un IC al 95% y si la teoría cae dentro) —ese es el corazón visual de la historia de validación."
              : "Who runs this replication framework in the lab matters, and it is honest to be precise: S01's live M/M/c run (SimPy) is a single ~300-customer run — it yields a point W_q, with no replications and no CI. The replicated-with-CI study proper is S10 (Monte-Carlo on joblib + SciPy): it runs n independent replications per regime and renders the CI band around the mean alongside the Erlang-C line, and the learner watches the band tighten as n grows (slider), sees the 1/√n law in action, and where the analytic value lands. S01 additionally carries a second-engine cross-check on Ciw (a capped 10-replication study reporting a 95% CI and whether the theory lands inside it) — that is the visual heart of the validation story."}
          </p>
    ...
    // ───────────────────────────────────────────────────────────────────────
    {
      ...
      content: (
        <div className="prose">
          ...
          <p>
            {es
              ? "Este laboratorio goza de ese patrón oro. La cola M/M/c es uno de los pocos modelos realistas con solución de estado estacionario en forma cerrada, la fórmula Erlang-C, que da la probabilidad exacta de que un cliente que llega deba esperar y, a partir de ella, el número medio exacto en cola L_q y la espera media W_q. La corrida M/M/c en vivo de S01 (SimPy) es una sola corrida de ~300 clientes: su W_q simulado puntual se contrasta directamente contra el Erlang-C cerrado (validación de punto único, sin barra de error). El protocolo de validación replicado-con-IC vive en otros dos lugares: (i) la verificación cruzada de segundo motor de Ciw que S01 incrusta en su propia traza —un estudio acotado y semillado de 10 réplicas que reporta el W_q medio, un medio-IC normal al 95% y si la teoría cae dentro (campos theory_in_ci y rel_err; no hay cadena «PASS»)— y (ii) el estudio Monte-Carlo S10 (joblib + SciPy), que ejecuta muchas réplicas independientes por régimen y calcula el IC de W_q (Sub-pestaña 5), confirmando que el valor analítico Erlang-C cae dentro del intervalo en los regímenes estables. Cualquier discrepancia es un error a cazar (verificación), no un juicio de modelado. El régimen inestable (ρ≥1) no tiene W_q de estado estacionario finito, así que no hay objetivo analítico que contrastar; sirve en cambio como prueba de condición extrema. Finalmente, todo el estudio se reporta según las guías STRESS (Monks et al., 2019): objetivos, la lógica exacta del modelo, todas las distribuciones y parámetros, el software y su versión, las semillas aleatorias y el número de réplicas —para que un tercero pueda regenerar cada número."
              : "This lab enjoys that gold standard. The M/M/c queue is one of the few realistic models with a closed-form steady-state solution, the Erlang-C formula, which gives the exact probability that an arriving customer must wait, and from it the exact mean number in queue L_q and mean wait W_q. S01's live M/M/c run (SimPy) is a single ~300-customer run: its point simulated W_q is checked straight against the closed-form Erlang-C (single-point validation, no error bar). The replicated-with-CI validation protocol lives in two other places: (i) the second-engine Ciw cross-check that S01 embeds in its own trace — a capped, seeded 10-replication study reporting the mean W_q, a normal 95% half-CI, and whether the theory lands inside it (fields theory_in_ci and rel_err; there is no \"PASS\" string) — and (ii) the S10 Monte-Carlo study (joblib + SciPy), which runs many independent replications per regime and computes the CI for W_q (Sub-tab 5), confirming the analytic Erlang-C value falls inside the interval across the stable regimes. Any discrepancy is a bug to hunt (verification) rather than a modeling judgment call. The unstable regime (ρ≥1) has no finite steady-state W_q, so there is no analytic target to check against; it instead serves as an extreme-condition test. Finally, the whole study is reported per the STRESS guidelines (Monks et al., 2019): objectives, the exact model logic, all distributions and parameters, the software and version, the random seeds, and the number of replications — so a third party can regenerate every number."}
          </p>

          <Callout variant="strong" title={es ? "La tesis del laboratorio" : "The lab's thesis"}>
            {es
              ? "Un simulador en el que se puede confiar porque reproduce una respuesta analítica conocida. Los 12 regímenes cubren ρ de carga ligera a saturación (uno inestable, sin objetivo finito); la figura muestra el W_q medio replicado con su IC del 95% —del estudio replicado (verificación cruzada de Ciw + Monte-Carlo S10), no de la corrida única de S01 en SimPy— superpuesto sobre la curva suave Erlang-C en los 11 regímenes estables: la prueba visual de la validación. El W_q simulado de la corrida única de S01 es un punto sin barra de error."
              : "A simulator to trust because it reproduces a known analytic answer. The 12 regimes span ρ from light to saturation (one unstable, with no finite target); the figure shows the replicated mean W_q with its 95% CI — from the replicated study (Ciw cross-check + S10 Monte-Carlo), not from S01's single SimPy run — overlaid on the smooth Erlang-C curve across the 11 stable regimes, the visual proof of validation. S01's single-run simulated W_q is a point estimate with no error bar."}
          </Callout>
          ...
          <p>
            {es
              ? "Esta es la tesis de CAOS_SIMLAB: un simulador en el que se puede confiar porque reproduce una respuesta analítica conocida. Los 12 regímenes cubren ρ de ligero a saturación (el inestable carece de objetivo finito); la figura muestra el W_q simulado con su IC —el del estudio replicado (la verificación cruzada de Ciw y el Monte-Carlo S10), no el de la corrida única de S01 en SimPy— superpuesto sobre la curva suave Erlang-C en los 11 regímenes estables, la prueba visual de validación. La misma pantalla lleva la tarjeta de reporte STRESS (semilla, versión, clientes por corrida, réplicas) para que la demostración sea ella misma reproducible."
              : "This is the thesis of CAOS_SIMLAB: a simulator to trust because it reproduces a known analytic answer. The 12 regimes span ρ from light to saturation (the unstable one has no finite target); the figure shows the simulated W_q with its CI — the CI comes from the replicated study (the Ciw cross-check and the S10 Monte-Carlo), not from S01's single SimPy run — overlaid on the smooth Erlang-C curve across the 11 stable regimes, the visual proof of validation. The same screen carries the STRESS report card (seed, version, customers per run, replications) so the demonstration is itself reproducible."}
          </p>
        ...
        <div className="prose">
          ...
          <div className="assume">
            ...
            <ul>
              {[
                es
                  ? "Elegir primero el régimen correcto. Aplicar calentamiento de estado estacionario a una pregunta terminante (o viceversa) responde el problema equivocado."
          ...
          <p>
            {es
              ? "Los regímenes M/M/c se ejecutan contra el objetivo de estado estacionario Erlang-C, y el estudio replicado (S10, Monte-Carlo) arranca cada réplica vacía y ociosa, lo que introduce sesgo de inicialización. Ese estudio no aplica un calentamiento explícito; en cambio toma n (clientes por corrida) lo bastante grande como para que el sesgo de inicialización sea despreciable frente a la media de toda la corrida, de modo que la estimación cae dentro del IC alrededor de la curva Erlang-C (y S10 expone además el caso ρ≈0.9 donde el sesgo de corrida finita no es despreciable, así que el IC se cierra en torno a un estimador sesgado). La figura ilustra el método general de Welch (suavizar el promedio de conjunto, localizar el aplanamiento ℓ, descartar el transitorio), no un control del laboratorio: muestra por qué un arranque vacío sesga a la baja y cómo se corregiría con truncamiento si se necesitara."
              : "The M/M/c regimes are run against the Erlang-C steady-state target, and the replicated study (S10, Monte-Carlo) starts each replication empty-and-idle, which introduces initialization bias. That study applies no explicit warm-up; instead it takes n (customers per run) large enough that the initialization bias is negligible relative to the whole-run mean, so the estimate lands inside the CI around the Erlang-C curve (and S10 additionally exposes the ρ≈0.9 case where the finite-run bias is not negligible, so the CI closes around a biased estimator). The figure illustrates the general Welch method (smooth the ensemble average, locate the flattening ℓ, discard the transient), not a lab control: it shows why an empty start biases low and how truncation would correct it if needed."}
          </p>
