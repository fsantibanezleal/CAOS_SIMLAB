      ...
      content: (
        <div className="th-block">
          <div className="prose">
            ...
            <p>
              {es
                ? "Activación simultánea (síncrona) — el régimen de este laboratorio. Todos los agentes calculan primero su próximo estado a partir de una instantánea congelada del tick anterior (fase step), y luego se confirman todos los estados juntos (fase advance). Ningún agente ve el movimiento de otro dentro del tick. Esto es exactamente lo que hacen los escenarios de agentes del laboratorio sobre Mesa 3: el paso por tick del modelo recorre el AgentSet (self.agents), decide todas las infecciones/recuperaciones (S03 SIR) o reubicaciones (S02 Schelling) leyendo el estado del inicio del paso, y recién entonces las aplica en bloque (Model.step() en S03/S05; un paso directo de reubicación en lote — batch relocate() — sobre el mismo AgentSet en S02). Es el régimen correcto cuando el modelo busca aproximar un sistema de ecuaciones en diferencias acopladas actualizadas en bloque — p. ej. flocking tipo autómata celular, donde cada boid reacciona a dónde estaban sus vecinos. Las actualizaciones síncronas pueden crear sus propios artefactos (dos agentes intercambiando celdas, oscilaciones tipo «blinker») que los esquemas asíncronos evitan."
                : "Simultaneous activation (synchronous) — this lab's regime. All agents first compute their next state from a frozen snapshot of the previous tick (a step phase), then all states are committed together (an advance phase). No agent sees another's move within the tick. This is exactly what the lab's Mesa 3 agent scenarios do: the model's per-tick pass iterates the AgentSet (self.agents), decides every infection/recovery (S03 SIR) or relocation (S02 Schelling) by reading the start-of-step state, and only then applies them in a batch (Model.step() in S03/S05; a direct batch relocate() pass over the same AgentSet in S02). This is the correct regime when the model is meant to approximate a system of coupled difference equations updated in lockstep — e.g. cellular-automaton style flocking where every boid reacts to where its neighbors were. Synchronous updates can create artifacts of their own (e.g. two agents swapping cells, “blinker” oscillations) that asynchronous schemes avoid."}
            </p>
            <p>
              {es
      ...
      content: (
        <div className="th-block">
          ...
          <figure className="figure">
            ...
            <figcaption className="fig-cap">
              {es
                ? "El bucle real del laboratorio: cada tick, el paso por tick de Mesa 3 recorre el AgentSet (self.agents), decide todas las transiciones contra el estado(t) y las aplica como actualización por lotes para producir el estado(t+1) (Model.step() en S03/S05; un paso directo de reubicación en lote en S02), se registra un frame, y la traza alimenta el visor de reproducción determinista. Este bucle de Mesa corre en vivo en Pyodide (S02/S03/S05); la traza canónica comprometida solo provee el primer cuadro instantáneo mientras el runtime calienta."
                : "The lab's actual loop: each tick, Mesa 3's per-tick pass iterates the AgentSet (self.agents), decides every transition against state(t) and applies them as a batch update to produce state(t+1) (Model.step() in S03/S05; a direct batch relocate() pass in S02), a frame is recorded, and the trace feeds the deterministic-replay viewer. This Mesa loop runs live in Pyodide (S02/S03/S05); the committed canonical trace only supplies the instant first paint while the runtime warms up."}
            </figcaption>
          </figure>
