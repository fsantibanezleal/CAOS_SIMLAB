        ...
        <div className="th-block">
          <div className="prose">
            ...
            <ul>
              ...
              <li>
                {es
                  ? "Activación por el AgentSet (self.agents) + actualización en bloque (batch update) — el paso del modelo recorre self.agents (el AgentSet de Mesa 3, sin los planificadores eliminados de Mesa 2), decide todas las transiciones contra la configuración del inicio del paso y las aplica en bloque; el matiz por escenario importa: en S02 (Schelling) los agentes descontentos se reubican uno a uno en el pool de vacías que crece (movimientos secuenciales), mientras que en S03 (SIR) las celdas cambian de estado de forma sincrónica. Toda la aleatoriedad fluye por el RNG sembrado de Mesa (Model(rng=seed) siembra self.random y self.rng), así run(params, seed) es una función pura — la base del visor de reproducción determinista."
                  : "Activation via the AgentSet (self.agents) + a batch update — the model's step iterates self.agents (Mesa 3's AgentSet, without Mesa 2's removed schedulers), decides every transition against the start-of-step configuration, then applies them in a batch; the per-scenario nuance matters: in S02 (Schelling) the unhappy agents relocate one-by-one into the growing empty pool (sequential moves), whereas in S03 (SIR) the cells flip state synchronously. All randomness flows through Mesa's seeded RNG (Model(rng=seed) seeds self.random and self.rng), so run(params, seed) is a pure function — the basis of the deterministic-replay viewer."}
              </li>
            </ul>
            <p>
              {es
